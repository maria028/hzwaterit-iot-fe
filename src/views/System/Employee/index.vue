            ...
            <CSearchTable
                ...
                <template #search>
                    ...
                    <CSearchBarItem label="手机号码">
                        <el-input
                            v-model="queryModel.phoneNumber"
                            clearable
                            maxlength="11"
                            placeholder="请输入手机号码" />
                    </CSearchBarItem>
                    ...
                    <el-table-column label="操作" fixed="right" min-width="200">
                        <template #default="scope">
                            ...
                            <el-button
                                type="primary"
                                text
                                @click="handlerUpdateStatus(scope.row.id, 1)"
                                v-if="scope.row.statusCode == '2'"
                                v-permission="'PUT/employee-status'"
                                >在职</el-button
                            >
                            <el-button
                                type="primary"
                                text
                                @click="handlerUpdateStatus(scope.row.id, 2)"
                            ...
                            <el-button
                                ...
                                v-permission="'PUT/employee-password'"
                                >重置密码</el-button
                            >
                        </template>
                    </el-table-column>
    ...
    <el-dialog
        ...
        <el-form :model="dialogData" ref="dialogFormRef" label-width="auto" :rules="dialogRules">
            ...
            <el-form-item label="手机号码" prop="phoneNumber">
                <el-input
                    v-model="dialogData.phoneNumber"
                    placeholder="请输入手机号码"
                    maxlength="11"
                    style="width: 240px" />
            </el-form-item>
            ...
            <el-form-item label="性别" prop="genderCode">
                <el-radio-group v-model="dialogData.genderCode">
                    <el-radio v-for="genderDict in genderDicts" :key="genderDict.code" :label="genderDict.code">{{
                        genderDict.name
                    }}</el-radio>
                </el-radio-group>
            </el-form-item>
...
const getDepartmentTree = () => {
    getDepartmentTreeData().then(async (response: Result<Dict[]> | any) => {
        ...
        await nextTick();
        treeRef.value.setCurrentKey(currentNodeKey);
    });
};
// 获取职位列表
const listPosition = () => {
    ...
    dialogFormRef.value.validate((valid: boolean) => {
        if (valid) {
            ...
            if (dialogData.value.id == 0) {
                addEmployee(dialogData.value).then(() => {
                    ElMessage.success("操作成功！");
                    dialogClose();
                    getTableData();
                });
            } else {
                updateEmployee(dialogData.value).then(() => {
                    ElMessage.success("操作成功！");
                    dialogClose();
                    getTableData();
                });
