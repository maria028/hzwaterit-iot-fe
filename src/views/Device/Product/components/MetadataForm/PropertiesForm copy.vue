<template>
    <el-form :model="form" ref="formRef" label-position="top" label-width="auto">
        ...
        >
            <el-input-number
                v-model="form.valueType.expands!.maxLength"
                :min="maxlength"
                :max="5000"
        ...
        <!-- 枚举值列表 -->
        <el-form-item
            v-if="['enum'].includes(form.valueType.type)"
            label="枚举项"
            prop="valueType.elements"
            :rules="[{ validator: validatorElements }]"
        >
        ...
        <!-- 布尔列表 -->
        <el-form-item
            v-if="['bool'].includes(form.valueType.type)"
            label="布尔值"
            prop="valueType.elements"
            :rules="[{ validator: validatorElements }]"
        >
            ...
            <el-button
                v-if="form.valueType.elements && form.valueType.elements.length < 2"
                :icon="Plus"
                style="width: 100%"
                @click="addElementsHandel"
            >
                添加布尔值
            </el-button>
        </el-form-item>
        ...
        >
            <div v-for="(item, index) in form.valueType.properties" :key="item.id" style="width: 100%" class="object-list">
                ...
                <el-popover :visible="propertiesFormVisibleFlag === index" :width="250" placement="bottom" trigger="click">
                    ...
                    <PropertiesForm :ref="(el) => setPropertiesFormRefs(el, index)" :formData="item" isChildJSON />
                </el-popover>
...
import { PropertiesBO, MetadataBasicBO, createInitData, MetadataColumnBO } from "@/types/metadata"
