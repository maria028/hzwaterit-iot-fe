 ...
 * @LastEditTime: 2024-04-28 14:11:49
 * @Description: 物模型属性表单
-->
<template>
    <div class="my-drawer">
        <el-drawer v-model="dialogVisible" direction="rtl" :show-close="false" :before-close="dialogClose">
            ...
            <el-form :model="formData" ref="dialogFormRef" label-position="top" label-width="auto">
                ...
                >
                    <el-input-number v-model="formData.valueType.expands!.maxLength" :min="maxlength" :max="5000" controls-position="right" step-strictly style="width: 100%" />
                </el-form-item>
                <!-- 时间格式化 -->
                <el-form-item
                ...
                <!-- 枚举值列表 -->
                <el-form-item v-if="['enum'].includes(formData.valueType.type)" label="枚举项" prop="valueType.elements" :rules="[{ validator: validatorElements }]">
                ...
                <!-- 布尔列表 -->
                <el-form-item v-if="['bool'].includes(formData.valueType.type)" label="布尔值" prop="valueType.elements" :rules="[{ validator: validatorElements }]">
                    <div v-for="(item, index) in formData.valueType.elements" :key="item.value" style="width: 100%" class="enum-list">
                        <el-space spacer=":">
                ...
                <!-- 子对象 -->
                <el-form-item
                    v-if="formData.valueType.type === 'object' || (formData.valueType.type === 'array' && formData.valueType.elementType === 'object')"
                    label="JSON对象"
        ...
        if (dialogVisible) {
            ...
            formData.value = {
                ...
                valueType: {
                    ...
                    scale: data.valueType?.scale || null, // 精度
                    elementType: data.valueType?.elementType || null, // 数组元素类型
                    elements: data.valueType?.elements || [{ text: "", value: "" }], //枚举项
                    expands: {
                        ...data?.valueType?.expands,
                        maxLength: data.valueType?.expands?.maxLength || null // 最大长度
                    }
                },
                expands: {
                    ...data?.expands,
...
watch(
    () => formData.value.valueType?.elements,
    (val) => {
        if (dialogFormRef.value) checkElements()
    },
    { deep: true }
)
// 枚举项检验
const checkElements = () => {
    dialogFormRef.value.validateField("valueType.elements", () => {})
}
// 枚举项/布尔值检验规则
const validatorElements = (rule: any, elements: any, callback: any) => {
    const type = formData.value.valueType.type === "bool" ? "布尔值" : "枚举值"
    if (elements.length === 0) {
        ...
        //含有空值
        callback(new Error(`请将${type}补充完整`))
    } else if (formData.value.valueType.type === "bool" && elements.length < 2) {
        // 布尔值必须2项
        callback(new Error(`请将${type}补充完整`))
    }
}
