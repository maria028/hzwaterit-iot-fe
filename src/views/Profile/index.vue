            ...
            <el-tab-pane label="个人信息" name="profile">
                <el-form :model="profileModel" label-width="auto">
                    ...
                    <el-form-item label="部门" v-if="profileModel.departmentNames.length > 0">
                        <el-tag v-for="departmentName in profileModel.departmentNames" :key="departmentName">{{
                            departmentName
                        }}</el-tag>
                    </el-form-item>
                    <el-form-item label="职位" v-if="profileModel.positionNames.length > 0">
                        <el-tag v-for="positionName in profileModel.positionNames" :key="positionName">{{
                            positionName
                        }}</el-tag>
                    </el-form-item>
                ...
                <el-form :model="profileModel" ref="profileForm" label-width="auto" :rules="profileFormRules">
                    ...
                    <el-form-item label="性别" prop="genderCode">
                        <el-radio-group v-model="profileModel.genderCode">
                            <el-radio
                                v-for="genderDict in genderDicts"
                                :value="genderDict.code"
                                :label="genderDict.code"
                                >{{ genderDict.name }}</el-radio
                            >
                        </el-radio-group>
                    </el-form-item>
            ...
            <el-tab-pane label="修改密码" name="updatePassword">
                <el-form :model="passwordModel" ref="passwordForm" label-width="auto" :rules="passwordFormRules">
                    <el-form-item label="原密码" prop="originalPassword">
                        <el-input
                            v-model="passwordModel.originalPassword"
                            placeholder="请输入原密码"
                            show-password
                            maxlength="16"
                            style="width: 240px" />
...
import ProfileBO from "@/types/ProfileBO";
...
// 获取个人信息
const getAccount = () => {
    getAccountProfile().then((response: Result<ProfileBO> | any) => {
